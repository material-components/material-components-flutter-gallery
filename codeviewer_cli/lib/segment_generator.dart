...
const _globalPrologue = '''// This file is automatically generated by codeviewer_cli.
...
_FileReadStatus _updatedStatus(_FileReadStatus oldStatus, String line) {
  ...
  switch (oldStatus) {
    case _FileReadStatus.comments:
      newStatus = (line.trim().isEmpty || lineStatus == _FileReadStatus.comments)
...
Map<String, String> _createSegments(String sourceDirectoryPath) {
  ...
  for (final file in files) {
    ...
    for (final line in lines) {
      ...
      if (line.trim().startsWith(beginSubsegment)) {
        String argumentString = line
            .replaceFirst(beginSubsegment, '')
            .trim();
        List<String> arguments = argumentString.isEmpty
            ? []
            : argumentString.split(RegExp(r'\s+'));

        for (final argument in arguments) {
          if (activeSubsegments.contains(argument)) {
            throw PreformatterException('BEGIN $argument is used twice in file ${file.path}');
        ...
        for (final argument in arguments) {
          if (activeSubsegments.contains(argument)) {
            ...
            throw PreformatterException('END $argument is used without a paired START in ${file.path}');
          }
        }
      } else {
        // Simple line.
  ...
  segments.forEach(
    (key, value) {
      value.sort(
        (ts1, ts2) => (ts1.order - ts2.order).sign.round()
      );
    }
  );
...
String _demoName(String path) {
  String fileName = path.split('/').last;
  String snakeCaseDemoName = fileName.replaceFirst('.dart', '');
  List<String> parts = snakeCaseDemoName.split('_');
  ...
  for (int i = 0; i < parts.length; i++) {
    if (i == 0) {
      ...
      result.write(_capitalize(parts[i]));
    }
  }
